...
foreach($get_html_contents->find('li.list-item') as $e){
	...
	foreach($f->find('a.item-subject') as $g){
		$targeturl = str_replace($base_url,"",$g->href);
		$targeturl = str_replace("/bbs/board.php?bo_table=comics&amp;wr_id=","",$targeturl);
		$ws_id = str_replace("&amp;spage=1","",$targeturl);
		$ws_id = str_replace($siteUrl,"",$ws_id);
		$epititle = trim(strip_tags($g));
		$epiurl = "/bbs/board.php?bo_table=comics&wr_id=".$ws_id;
