...
//	echo "<script><!--".$get_html_contents."//--></script>";
//	echo "<script type=text/javascript>console.log('".$get_html_contents."');</script>";

	foreach($get_html_contents->find('h1') as $e){
		$epititle = $e->content;
	}
	foreach($get_html_contents->find('a') as $e){
		if($e->getAttribute("data-original-title") == "<nobr>이전화</nobr>"){
			$prev_url = $e->href;
			$prevurl = str_replace($siteUrl,"",$prev_url);
			$prevurl = str_replace("./board.php?bo_table=webtoon&amp;wr_id=","",$prevurl);
		}
		if($e->getAttribute("data-original-title") == "<nobr>다음화</nobr>"){
			$next_url = $e->href;
			$nexturl = str_replace($siteUrl,"",$next_url);
			$nexturl = str_replace("./board.php?bo_table=webtoon&amp;wr_id=","",$nexturl);
		}
	}
