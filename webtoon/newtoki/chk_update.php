...
<?php
	function chkRecent($SiteId, $ToonId, $EpiId, $siteUrl, $viewUrl) {
		...
		foreach($get_html_contents->find('a#goNextBtn') as $e){
			$next_epi = $e->href;
			$next_epi = str_replace("#next","",$next_epi);

			$nextparse = explode('/' , $next_epi);
			$tempparse = explode('?', $nextparse[4]);
			$next_epi = $tempparse[0];
			break;
		}
...
<?php
	...
	if ( strlen($recentToons) > 1 ) {
		send_discord($webhookURL, "뉴토끼[NEWTOKI] 웹툰이 업데이트되었습니다.", "Jackie Choi", "뉴토끼[NEWTOKI] 웹툰 업데이트 내역", $recentToons, "http://armtoon.enas.kr/webtoon/newtoki/", "3366ff");
	}
?>
