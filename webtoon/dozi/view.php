<?php
	...
	$epiurl = str_replace("{toonid}",$_GET["wr_id"],$viewUrl);
	$epiurl = str_replace("{toondtlid}",$_GET["ws_id"],$epiurl);
	$title = $_GET["title"];
	$next_epi = "";
	...
	foreach($get_html_contents->find('div.btn-resource') as $e){
		$htmltxt = str_get_html($e->innertext);

		if($e->getAttribute("data-placement") == "right"){
			$prevurl = $htmltxt->find('a')[0];
			$full_url = str_replace("/webtoon/","",$prevurl->href);
			$prev_ws = str_replace($_GET["wr_id"],"",$full_url);
			$prev_ws = str_replace("/","",$prev_ws);
			$prev_wr = $_GET["wr_id"];
			$prev_url = "view.php?title=".urlencode($title)."&wr_id=".$prev_wr."&ws_id=".$prev_ws;
		}
		if($e->getAttribute("data-placement") == "left"){
			$nexturl = $htmltxt->find('a')[0];
			$full_url = str_replace("/webtoon/","",$nexturl->href);
			$next_ws = str_replace($_GET["wr_id"],"",$full_url);
			$next_ws = str_replace("/","",$next_ws);
			$next_wr = $_GET["wr_id"];

			$next_url = "view.php?title=".urlencode($title)."&wr_id=".$next_wr."&ws_id=".$next_ws;
		}
	}
			...
			<dd>
				<div class='group' style='padding:0px;'>
					...
					<tr style='background-color:#f8f8f8'>
						...
						<?php
							if( $prev_url != null && strlen($prev_url) > 0 ){
...
<?php
	...
	$conts_arr = explode("var tnimg = ", $get_html_contents);
