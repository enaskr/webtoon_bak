	...
	function chkRecent($SiteId, $ToonId, $EpiId, $siteUrl, $viewUrl) {
		...
		foreach($get_html_contents->find('a') as $e){
			if($e->getAttribute("data-original-title") == "<nobr>다음화</nobr>"){
				$next_epi = $e->href;
				$next_epi = str_replace($base_url,"",$next_epi);
				$next_epi = str_replace("./board.php?bo_table=comics&amp;wr_id=","",$next_epi);
				break;
			}
		}
...
<?php
	...
	$isAlreadyView = $isAlreadyView." WHERE UV.MBR_NO = '".$MBR_NO."' AND UV.SITE_ID = '".$SITE_ID."' AND UV.USE_YN='Y' ";
	...
	while($row = $webtoonView->fetchArray(SQLITE3_ASSOC)){
		...
		$epiid = $row["VIEW_ID"];
		$recentResult = chkRecent($SITE_ID,$toonid,$epiid, $siteUrl, $viewUrl);
		if ( $recentResult ) {
	...
	if ( strlen($recentToons) > 1 ) {
		send_discord($webhookURL, "19올넷[19ALLNET] 웹툰이 업데이트되었습니다.", "Jackie Choi", "19올넷[19ALLNET] 웹툰 업데이트 내역", $recentToons, "http://armtoon.enas.kr/webtoon/19allnetw/", "3366ff");
	}
?>
